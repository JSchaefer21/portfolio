...
function Game({ onBackClicked, onResetClicked }: Props) {
    ...
    const random = () => {
        return Math.floor(Math.random() * (max.current - min.current + 1) + min.current)
    }
    ...
    return <>
        <div id='game' className='Game'>
            {isInfoVisible && <GameInfo onCloseClicked={() => setInfoVisible(false)} />}

            <div className='Game_container-buttons'>
                <button className='Game_button' onClick={onBackClicked}>Back</button>
